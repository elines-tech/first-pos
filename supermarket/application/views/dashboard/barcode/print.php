				...
				<div class="col-12 col-md-6 order-md-2 order-first">
					<nav aria-label="breadcrumb" class="breadcrumb-header float-start float-lg-end">
						<ol class="breadcrumb">
							...
							<li class="breadcrumb-item active" aria-current="page"><?php echo $translations['Barcode']?></li>
						</ol>
					</nav>
				</div>
			</div>
	...
	function generate() {
		...
		if (lineCode != '' && language != '' && barcodeQty != '') {
			if (barcodeQty > 0) {
				...
				toastr.error('Barcode Quantity should be greater than zero', 'Barcode print', {
					"progressBar": false
				});
				$('#barcodeQty').val('');
				$('#barcodeQty').focus();
			...
			toastr.error('* fields are required', 'Barcode print', {
				"progressBar": false
			});
			return false;
		}
