...
class Branch extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->rights = $this->GlobalModel->getMenuRights('7.1', $rolecode);
		if ($this->rights == '') {
	...
	public function getBranchList()
	{
		...
		$like = array("branchmaster.branchName" => $search . "~both", "branchmaster.code" => $search . "~both", "taxgroupmaster.taxGroupName" => $search . "~both");
		...
		if ($Records) {
			foreach ($Records->result() as $row) {
				...
				if ($this->rights != '' && $this->rights['view'] == 1) {
					$actionHtml .= '<a id="view" class="btn btn-success btn-sm  m-1 cursor_pointer" href="' . base_url() . 'branch/view/' . $row->code . '"><i id="edt" title="View" class="fa fa-eye"></i></a>';
				}
				if ($this->rights != '' && $this->rights['update'] == 1) {
					$actionHtml .= '<a id="edit" class="btn btn-info btn-sm m-1 cursor_pointer edit_branch" data-seq="' . $row->code . '"><i id="edt" title="Edit" class="fa fa-pencil"></i></a>';
				}
				if ($this->rights != '' && $this->rights['delete'] == 1) {
					$actionHtml .= '<a id="delete" class="btn btn-sm btn-danger m-1 cursor_pointer delete_branch" data-seq="' . $row->code . '"><i id="dlt" title="Delete" class="fa fa-trash"></i></a>';
				}
				$actionHtml .= '</div>';
				$data[] = array(
					$srno,
