...
class Offer extends CI_Controller
{
	...
	public function add()
	{
		if ($this->rights != '' && $this->rights['insert'] == 1) {
			$this->load->view('dashboard/header');
			$this->load->view('dashboard/offer/add');
			$this->load->view('dashboard/footer');
		} else {
			$this->load->view('errors/norights.php');
		}
	...
	public function getOfferList()
	{
		...
		if ($Records) {
			foreach ($Records->result() as $row) {
				...
				$actionHtml = '';
				if ($this->rights != '' && $this->rights['view'] == 1) {
					$actionHtml .= '<a href="' . base_url() . 'offer/view/' . $row->code . '" class="btn btn-success btn-sm cursor_pointer m-1"><i id="view" title="View" class="fa fa-eye"></i></a>';
				}
				if ($this->rights != '' && $this->rights['update'] == 1) {
					$actionHtml .= '<a href="' . base_url() . 'offer/edit/' . $row->code . '" class="btn btn-info btn-sm m-1 cursor_pointer"><i id="edt" title="Edit" class="fa fa-pencil"></i></a>';
				}
				if ($this->rights != '' && $this->rights['delete'] == 1) {
					$actionHtml .= '<a class="btn btn-danger btn-sm m-1 cursor_pointer delete_offer" id="' . $row->code . '"><i id="dlt" title="Delete" class="fa fa-trash"></i></a>';
				}
				$data[] = array(
					$srno,
					$row->code,
	...
	public function save()
	{
		...
		$data = array(
			...
			'startDate' => date('Y-m-d H:i:00', strtotime($this->input->post("startDate"))),
			'endDate' => date('Y-m-d H:i:00', strtotime($this->input->post("endDate"))),
			'addID' => $addID,
			'addIP' => $ip,
			'isActive' => $isActive
		);
