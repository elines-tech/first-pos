...
class Subscriptions extends CI_Controller
{
    ...
    public function modify()
    {
        ...
        $data['subscription'] = $this->AdminModel->selectQuery("*", "subscriptionmaster", ["code" => "PAK_1"], ["id" => "ASC"]);
        $this->load->view('dashboard/header');
    ...
    public function upgrade()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($resultCode != 'false') {
                ...
                $this->session->set_flashdata("msg", "Transaction Successful");
    ...
    public function callback()
    {
        ...
        if ($status == "Paid") {
            if (!empty($payment)) {
                ...
                $message = "Your purchase is successful but failed to update on our server. Please contact administrator for futher queries. Transaction Id $keyId";
                $this->session->set_flashdata("message", json_encode(["status" => true, "message" => $message]));
                redirect("subscriptions/listRecords", 'refresh');
            }
        } else {
