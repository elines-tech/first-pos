...
class Tax extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->rights = $this->GlobalModel->getMenuRights('7.2', $rolecode);
		if ($this->rights == '') {
	...
	public function getTaxGroupList()
	{
		...
		$like = array("taxgroupmaster.taxes" => $search . "~both", "taxgroupmaster.code" => $search . "~both", "taxgroupmaster.taxGroupName" => $search . "~both", "taxgroupmaster.taxGroupRef" => $search . "~both");
		$Records = $this->GlobalModel->selectQuery($orderColumns, $tableName, $condition, $orderBy, $join, $joinType, $like, $limit, $offset, $groupByColumn, $extraCondition);
		$srno = $_GET['start'] + 1;
		if ($Records) {
			foreach ($Records->result() as $row) {
