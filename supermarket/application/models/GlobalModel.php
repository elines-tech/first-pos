...
class GlobalModel extends CI_Model
{
    public function __construct()
    {
        ...
        try {
            ...
            if (isset($this->session->userdata['current_db' . $session_key]) && $this->session->userdata['current_db' . $session_key] != "") {
                $dbName = $this->session->userdata['current_db' . $session_key];
                $dynamicDB = array(
    ...
    public function selectApprovedData($tblname)
    {
        $query = $this->db->query("SELECT * FROM `" . $tblname . "` WHERE `isActive` = '1' and `isApproved` = '1'");
        return $query;
    }
    ...
    public function doEditForOrders($data, $tblname, $field, $code)
    {
        $nowdate = date('Y-m-d H:i:s');
        //$editDate = array('editDate' => $nowdate);
        //$data = array_merge($data, $editDate);
    ...
    public function updateStock($productCode, $variantCode, $returnQty, $branchCode, $batchNo)
    {
        $consumeQty = $factorConversionStockQty = $newConversionStockQty = $actualStock = 0;
        $this->db->select("stockinfo.code,ifnull(stockinfo.stock,0) as stock,stockinfo.productCode,stockinfo.variantCode,stockinfo.branchCode,stockinfo.batchNo");
        $this->db->from("stockinfo");
        $this->db->where('stockinfo.productCode', $productCode);
        $this->db->where('stockinfo.variantCode', $variantCode);
        $this->db->where('stockinfo.branchCode', $branchCode);
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            $stockQty = $query->result_array()[0]['stock'];
            $code = $query->result_array()[0]['code'];
            $this->db->select("orderlineentries.qty as orderQty");
            $this->db->from("orderlineentries");
            $this->db->join("ordertemp", "ordertemp.orderId = orderlineentries.orderId");
            $this->db->where("orderlineentries.productCode", $productCode);
            $this->db->where("orderlineentries.variantCode", $variantCode);
            $this->db->where("ordertemp.branchCode", $branchCode);
            $getOrderQty = $this->db->get();
            if ($getOrderQty->num_rows() > 0) {
                $orderQty = $getOrderQty->result_array()[0]['orderQty'];
                $this->db->select("productmaster.ingredientFactor");
                $this->db->from("productmaster");
                $this->db->where("productmaster.productCode", $productCode);
                $getingredientFactor = $this->db->get();
                if ($getingredientFactor->num_rows() > 0) {
                    $ingredientFactor = $query->result_array()[0]['ingredientFactor'];
                    $consumeQty = $orderQty * $returnQty;
                    $factorConversionStockQty = $stockQty * $ingredientFactor;
    ...
    public function getMenuRights($menuId, $roleCode)
    {
        ...
        if ($cmpcode) {
            if (file_exists(APPPATH . 'rights/' . $cmpcode . '/' . $roleCode . '.json')) {
                ...
                copy(APPPATH . "upload/default.json", $copyfile);
                $rightscontents = file_get_contents(APPPATH . 'rights/' . $cmpcode . '/' . $roleCode . '.json');
            }
    ...
    public function checkfirstSubscrription()
    {
        ...
        if (!empty($companymaster)) {
            //when onboard == 1 then onboarding is not complete
    ...
    {
        $cardno = $this->randomCharacters(12);
        $res = $this->db->select("id")->from("salegiftcardlineentries")->where("barcodeText", $cardno)->count_all_results();
        if ($res > 0) {
            $this->generateCardNo();
        }
        return $cardno;
