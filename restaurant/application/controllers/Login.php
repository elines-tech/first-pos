...
class Login extends CI_Controller
{
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($clientCode == 'false') {
                ...
                if ($CompanyDbResult != false) {
                    ...
                    if (!empty($result)) {
                        $mailsettings = $this->GlobalModel->selectQuery("settings.*", "settings", ["code" => "STG_3"]);
                        if ($mailsettings) {
    ...
    public function try()
    {
        $config = [
            "maildriver" => "smtp",
            "host" => "tamakan.com.sa",
            "username" => "service@tamakan.com.sa",
            "password" => "service@123415678",
            "port" => 465,
            "fromaddress" => "service@tamakan.com.sa",
            "fromname" => "TAMAKAN SOFTWARE"
        ];
        $toMailAddress = "testing.neosaoservices@gmail.com";
        $subject = "Dynamic Mail Sending";
        $message = "Design Patterns….??? I think you have heard this name before in programming… Yes, you might have heard this name before in programming if you are…";
        print_r($this->emailer->transact($config, $toMailAddress, $subject, $message, []));
    }
