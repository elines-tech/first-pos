...
class Unit extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->rights = $this->GlobalModel->getMenuRights('8.8', $rolecode);
		if ($this->rights == '') {
	...
	public function getUnitList()
	{
		...
		$orderColumns = array("unitmaster.code,unitmaster.unitName,unitmaster.unitSName,unitmaster.isActive");
		$condition = array();
		...
		$like = array("unitmaster.code" => $search . "~both","unitmaster.unitName" => $search . "~both", "unitmaster.unitSName" => $search . "~both");
		...
		if ($Records) {
			foreach ($Records->result() as $row) {
				...
				if ($this->rights != '' && $this->rights['view'] == 1) {
					$actionHtml .= '<a id="view" class="edit_unit btn btn-success btn-sm cursor_pointer m-1" data-seq="' . $row->code . '" data-type="1"><i id="edt" title="View" class="fa fa-eye"></i></a>';
				}
				if ($this->rights != '' && $this->rights['update'] == 1) {
					$actionHtml .= '<a id="edit" class="edit_unit btn btn-info btn-sm cursor_pointer m-1" data-seq="' . $row->code . '" data-type="2"><i id="edt" title="Edit" class="fa fa-pencil"></i></a>';
				}
				if ($this->rights != '' && $this->rights['delete'] == 1) {
					$actionHtml .= '<a id="delete" class="btn btn-sm btn-danger delete_unit m-1" data-seq="' . $row->code . '"><i id="dlt" title="Delete" class="fa fa-trash"></i></a></div>';
				}
	...
	public function editUnit()
	{
		...
		$unitQuery = $this->GlobalModel->selectQuery("unitmaster.code,unitmaster.unitName,unitmaster.unitSName,unitmaster.unitDesc,unitmaster.isActive", 'unitmaster', array('unitmaster.code' => $code));
		if ($unitQuery) {
	...
	public function deleteUnit()
	{
		...
		if ($query) {
			$txt = $code . " - " . $tableNumber . " unit is deleted.";
			$activity_text =  $date . "\t" . $ip . "\t" . $userRole . "\t" . $userName . "\t" . $txt;
			$this->GlobalModel->activity_log($activity_text);
		}
		echo $query;
