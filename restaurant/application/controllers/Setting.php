...
class Setting extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->rights = $this->GlobalModel->getMenuRights('8.10', $rolecode);
		if ($this->rights == '') {
	...
	public function servicecharge()
	{
		...
		if ($this->rights != '' && $this->rights['update'] == 1) {
			...
			$data = array(
				"id" => 1,
				"code" => "STG_1",
				'settingName' => "Order service charges",
			...
			if ($result) {
				$txt = "Textsms Setting is updated.";
	...
	public function update_email()
	{
		...
		if ($this->rights != '' && $this->rights['update'] == 1) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$encryption = trim($this->input->post('encryption'));
	...
	public function twilio()
	{
		...
		if ($this->rights != '' && $this->rights['update'] == 1) {
			...
			$hasRecord = $this->GlobalModel->get_row_array(["code" => $settingCode], "settings");
			if (!empty($hasRecord)) {
				$data["editID"] = $addID;
				$data["editIP"] = $ip;
				$result = $this->GlobalModel->doEdit($data, 'settings', $settingCode);
			} else {
				$data["id"] = 4;
				$data["code"] = $settingCode;
				$data["addID"] = $addID;
				$data["addIP"] = $ip;
				$result = $this->GlobalModel->directInsert($data, 'settings');
			}
