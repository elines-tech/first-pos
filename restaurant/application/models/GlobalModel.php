...
class GlobalModel extends CI_Model
{
    ...
    {
        $condition = "`userName` ='" . $data['username'] . "' AND `userPassword` = '" . $data['password'] . "' AND `isActive` = '1'";
        $this->db->select('*');
    ...
    // Read data using username and loginpin
    public function loginWithPin($data)
    {
        $condition = "`userName` ='" . $data['userName'] . "' AND `loginpin` = '" . $data['loginpin'] . "' AND `isActive` = '1'";
        $this->db->select('*');
        $this->db->from('usermaster');
        $this->db->where($condition);
        $this->db->limit(1);
    ...
    public function login_pin($data)
    {
        $condition = "`loginpin` ='" . $data['loginpin'] . "' AND `isActive` = '1'";
        $this->db->select('*');
        $this->db->from('usermaster');
        $this->db->where($condition);
        $this->db->limit(1);
    ...
    public function read_user_information_withpin($loginpin)
    {
        $condition = "loginpin =" . "'" . $loginpin . "'";
        $this->db->select('*');
        $this->db->from('usermaster');
        $this->db->where($condition);
        $this->db->limit(1);
    ...
    public function checkDuplicateRecordNew(array $condition, $tablename)
    {
        $this->db->where($condition);
    ...
    function plainQuery(string $query_string)
    {
        return $this->db->query($query_string);
    }

    function directInsert($data, $table)
    {
        return $this->db->insert($table, $data);
    }

    function get_data_by_field($code, $tablename)
    {
        $query = $this->db->where('itemCode', $code)->get($tablename);
        if ($query->num_rows() > 0) {
            return $query->result_array();
        } else {
            return false;
    ...
    function get_data($code, $tablename)
    {
        $query = $this->db->where('code', $code)->get($tablename);
        if ($query->num_rows() > 0) {
            return $query->result_array();
        } else {
            return false;
    ...
    public function activity_log($data)
    {
        ...
        $destinationPath = "upload/activitylog";
    ...
    function checkrecord_exists($columnName, $value, $tablename)
    {
    ...
    public function getMaxIdBeforeInsert($tblname)
    {
    ...
    {
        $ret = '';
        $cmpcode = $this->getCompcode();
        if ($cmpcode) {
            if (file_exists(APPPATH . 'rights/' . $cmpcode . '/' . $roleCode . '.json')) {
                $rightscontents = file_get_contents(APPPATH . 'rights/' . $cmpcode . '/' . $roleCode . '.json');
            } else {
                $createfile = fopen(APPPATH . 'rights/' . $cmpcode . '/' . $roleCode . '.json', 'w');
                $copyfile = APPPATH . 'rights/' . $cmpcode . '/' . $roleCode . '.json';
                $basefile = base_url("upload/default.json");
                $file = file_get_contents($basefile);
                file_put_contents($copyfile, $file);
                $rightscontents = file_get_contents(APPPATH . 'rights/' . $cmpcode . '/' . $roleCode . '.json');
            }
    ...
    {
        $data = $this->db->select("companymaster.code")->from("companymaster")->limit(1)->get()->row_array();
        if (!empty($data)) {
            return $data['code'];
        }
        return false;
    ...
    {
        $companymaster = $this->db->select('code')->from("companymaster")->get()->row_array();
        if (!empty($companymaster)) {
            $clientCode = $companymaster['code'];
            return  $this->planchecker->checkexpiry($clientCode);
        }
    }

    public function checkfirstSubscrription()
    {
        //$session_key = $this->session->userdata('key' . SESS_KEY);
        $companymaster = $this->db->select('onBoard')->from("companymaster")->get()->row_array();
        if (!empty($companymaster)) {
            if ($companymaster['onBoard'] == 1) {
                return true;
            }
        }
        return false;
    ...
    public function getLoginClientCode()
    {
        return $this->db->select("code as companyCode")->from("companymaster")->limit(1)->get()->row_array();
    }

    public function consumeStkQty($productCode, $qty, $branchCode)
    {
        $result = $this->db->query("SELECT IFNULL(code,'-') as rCode from recipecard where recipecard.productCode='$productCode'");
        if ($result->num_rows() > 0) {
            $recipe = $result->result_array()[0];
            $recipeCode = $recipe['rCode'];
            $recipeItems = $this->db->query("SELECT recipelineentries.itemCode, recipelineentries.itemQty as ingrediantQty,IFNULL(itemmaster.ingredientFactor,0) as ingfac FROM recipelineentries INNER JOIN itemmaster on recipelineentries.itemCode=itemmaster.code where recipelineentries.recipeCode='$recipeCode'");
            if ($recipeItems->num_rows() > 0) {
                $ritems = $recipeItems->result();
                foreach ($ritems as $r) {
                    $ingrediantQty = $r->ingrediantQty;
                    $itemCode = $r->itemCode;
                    $ingredientFactor = $r->ingfac;
                    $consumeQty = $ingrediantQty * $qty;
                    if ($ingredientFactor > 0) {
                        $stock = $this->db->query("SELECT IFNULL(stockinfo.stock,0) as stockQty from stockinfo where stockinfo.itemCode='$itemCode' AND stockinfo.branchCode='$branchCode'")->row_array();
                        if (!empty($stock)) {
                            $factorConversionStockQty = $stock['stockQty'] * $ingredientFactor;
                            $newConversionStockQty = $factorConversionStockQty - $consumeQty;
                            $actualStock = $newConversionStockQty / $ingredientFactor;
                            log_message("error", "stock consumption for product $productCode  itemcode => $itemCode, consumeQty => $consumeQty, fullStock => $factorConversionStockQty, remainingStock => $newConversionStockQty");
                            $this->db->query("UPDATE stockinfo SET stock='$actualStock' where stockinfo.itemCode='$itemCode' AND stockinfo.branchCode='$branchCode'");
                        }
                    }
                }
            }
    ...
    public function consumeComboProductStkQty($comboProducts, $qty, $branchCode)
    {
        //[{"productCode":"PRO22_3","productQty":"1"},{"productCode":"PRO22_2","productQty":"1"}]
        foreach ($comboProducts as $comboProduct) {
            $productCode = $comboProduct->productCode;
            //echo "comoboProductCode => $productCode <br>";
            //echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;*************************************************************<br>";
            $productQty = $comboProduct->productQty;
            $result = $this->db->query("SELECT IFNULL(code,'-') as rCode from recipecard where recipecard.productCode='$productCode'");
            if ($result->num_rows() > 0) {
                $recipe = $result->result_array()[0];
                $recipeCode = $recipe['rCode'];
                ...
                if ($recipeItems->num_rows() > 0) {
                    ...
                    foreach ($ritems as $r) {
                        ...
                        $itemCode = $r->itemCode;
                        $ingredientFactor = $r->ingfac;
                        $consumeQty = $ingrediantQty * $qty * $productQty;
                        if ($ingredientFactor > 0) {
                            $stock = $this->db->query("SELECT IFNULL(stockinfo.stock,0) as stockQty from stockinfo where stockinfo.itemCode='$itemCode' AND stockinfo.branchCode='$branchCode'")->row_array();
                            if (!empty($stock)) {
                                $factorConversionStockQty = $stock['stockQty'] * $ingredientFactor;
                                $newConversionStockQty = $factorConversionStockQty - $consumeQty;
                                $actualStock = $newConversionStockQty / $ingredientFactor;
                                log_message("error", "stock consumption for [combo] product $productCode :  itemcode => $itemCode, consumeQty => $consumeQty, fullStock => $factorConversionStockQty, remainingStock => $newConversionStockQty");
                                $this->db->query("UPDATE stockinfo SET stock='$actualStock' where stockinfo.itemCode='$itemCode' AND stockinfo.branchCode='$branchCode'");
                            }
                        }
                    }
    ...
    public function consumeAddonStkQty($addOns, $branchCode)
    {
        //[{"addonCode":"PEXT22_1","addonTitle":"Oregano","addonPrice":"8.00","addonQty":"1"}]
        foreach ($addOns as $addOn) {
            $addonCode = $addOn->addonCode;
            $addonQty = $addOn->addonQty;
            //echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;addOnCode => $addonCode <br>";
            //echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;*************************************************************<br>";           
            $select = "productextras.itemQty, itemmaster.ingredientFactor, productextras.itemCode";
            $table = "productextras";
            $condition = ["productextras.code" => $addOn->addonCode];
            $orderBy = [];
            $joinType = ["itemmaster" => "inner"];
            $join = ["itemmaster" => "productextras.itemCode = itemmaster.code"];
            $actualItems = $this->selectQuery($select, $table, $condition, $orderBy, $join, $joinType);
            if ($actualItems) {
                $actualItems = $actualItems->result();
                foreach ($actualItems as $actualItem) {
                    $itemCode = $actualItem->itemCode;
                    $consumeQty = $addonQty * $actualItem->itemQty;
                    $ingredientFactor = $actualItem->ingredientFactor ?? 0;
                    if ($ingredientFactor > 0) {
                        $stock = $this->db->query("SELECT IFNULL(stockinfo.stock,0) as stockQty from stockinfo where stockinfo.itemCode='$itemCode' AND stockinfo.branchCode='$branchCode'")->row_array();
                        if (!empty($stock)) {
                            $factorConversionStockQty = $stock['stockQty'] * $ingredientFactor;
                            $newConversionStockQty = $factorConversionStockQty - $consumeQty;
                            $actualStock = $newConversionStockQty / $ingredientFactor;
                            log_message("error", "stock consumption for addon $addonCode : itemcode => $itemCode, consumeQty => $consumeQty, fullStock => $factorConversionStockQty, remainingStock => $newConversionStockQty");
                            $this->db->query("UPDATE stockinfo SET stock='$actualStock' where stockinfo.itemCode='$itemCode' AND stockinfo.branchCode='$branchCode'");
                        }
                    }
                }
    ...
    public function hasSimilarRecords(array $where, string $table)
    {
        $count = $this->db->select('*')->from($table)->where($where)->count_all_results();
        if ($count > 0) {
            return true;
        }
        return false;
    ...
    public function get_row_array(array $where, string $table)
    {
        return $this->db->select($table . ".*")->where($where)->get($table)->row_array();
    }

    public function delete_by_where($where, $table)
    {
