...
<body>
  ...
  <?php 
   ...
   <input type="hidden" id="mobilenumber" name="mobilenumber[]" value="<?= $number ?>">
  <?php 
  $i++;
	 }}
  ?>
...
<script>
  $(document).ready(function() {
	   ...
	   var mobilenumber = $("input[name='mobilenumber[]']")
        ...
        var size = 10;
	    var arrayOfArrays = [];
        for (var i=0; i<mobilenumber.length; i+=size) {
             arrayOfArrays.push(mobilenumber.slice(i,i+size));	
        }
	...
	{
		 $.ajax({
			url: base_path+"sendsms/sendSmsNotification",  
			method:"POST",
			data:{message,mobileNumber},
			datatype:"text",
			async: false,
	  ...
	  function progress_bar_process(percentage)
	  {
	   ...
	   {	
		...
		$('#success_message').html("<div class='alert alert-success'>All Sms Sent Successfully</div>");
		setTimeout(function(){
			window.location.href=base_path+"sendsms/notifiy";
		}, 5000);
	   }
	  }
