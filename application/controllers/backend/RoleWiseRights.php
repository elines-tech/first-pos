...
class RoleWiseRights extends CI_Controller
{
	...
	{
		$data['roles'] = $this->GlobalModel->selectActiveData('rolesmaster');
		$this->load->view('backend/template/header');
		$this->load->view('backend/template/sidebar');
		$this->load->view('backend/rolewiserights/list',$data);
		$this->load->view('backend/template/footer');
	}
	
	public function getMenuList(){
		$role= $this->input->post("role");
		if(file_exists('assets/rights/menu.json')){
			$filecontents = file_get_contents('assets/rights/menu.json');
			$menuJson = json_decode($filecontents,true);
			$filename='assets/rights/'.$role.'.json';
			$rightJson=[];
