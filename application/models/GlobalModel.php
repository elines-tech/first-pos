...
class GlobalModel extends CI_Model
{
    ...
    {
        //$condition = "`userName` ='" . $data['userName'] . "' AND `userPassword` = '" . $data['userPassword'] . "' AND `isActive` = '1' AND `userRole` = 'ADMIN'";
    ...
    {
        $condition = "`loginpin` ='" . $data['loginpin'] . "' AND `isActive` = '1' AND `userRole` = 'R_3'";
        $this->db->select('*');
        $this->db->from('usermaster');
        $this->db->where($condition);
        $this->db->limit(1);
    ...
    public function checkDuplicateRecordNew(array $condition, string $tablename)
    {
        $this->db->where($condition);
    ...
    function get_data_by_given_field($field, $code, $tablename)
    {
        $query = $this->db->where($field, $code)->get($tablename);
        if ($query->num_rows() > 0) {
            return $query->result_array();
        } else {
            return false;
